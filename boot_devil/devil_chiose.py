...
class NeuralNetwork(nn.Module):
    ...
    def model_saver(self,model):
    	torch.save(model.linear_relu_stack.state_dict(), "weight_devil/devil_choice.pt")
    def model_loder(self,model):
    	model.linear_relu_stack.load_state_dict(torch.load("weight_devil/devil_choice.pt"))


model = NeuralNetwork().to(device)
#model.train_devil(model)
#model.model_saver(model)
model.model_loder(model)
devil_t=float(input(":"))
for i in range(10):
	devil=model(torch.from_numpy(numpy.array([devil_t]).astype('float32'))).detach()
	devil_t=devil_sub.devil_in(devil,devil_t)
	print(devil_sub.devil_high_ras([i for i in zip(range(1,6),numpy.array(torch.sin(devil)))])[0])
